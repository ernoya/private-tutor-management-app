...
const SUBJECT_COLORS: Record<string, string> = {
  ...
  "أحياء": "bg-emerald-100 text-emerald-700",
  "عربي": "bg-orange-100 text-orange-700",
  "انجليزي": "bg-sky-100 text-sky-700",
...
export default function GroupsPage() {
  ...
  const openAdd = () => {
    setForm({
      name: "", subject: "رياضيات", grade: "", monthlyFee: "",
      maxStudents: "20", notes: "",
      schedules: [{ dayOfWeek: "السبت", startTime: "16:00", endTime: "17:30" }],
    });
    setEditMode(false);
    setShowModal(true);
  };
  ...
  const openEdit = (g: Group) => {
    setForm({
      ...
      subject: g.subject,
      grade: g.grade || "",
      monthlyFee: parseFloat(g.monthlyFee).toString(),
      maxStudents: (g.maxStudents || 20).toString(),
      notes: g.notes || "",
      schedules: g.schedules.length > 0 ? g.schedules : [{ dayOfWeek: "السبت", startTime: "16:00", endTime: "17:30" }],
    });
    setEditMode(true);
  ...
  const addSchedule = () => {
    setForm((f) => ({
      ...f,
      schedules: [...f.schedules, { dayOfWeek: "السبت", startTime: "16:00", endTime: "17:30" }],
    }));
  };
  ...
  const updateSchedule = (i: number, field: keyof Schedule, value: string) => {
    setForm((f) => ({
      ...f,
      schedules: f.schedules.map((s, idx) => idx === i ? { ...s, [field]: value } : s),
    }));
  };
  ...
  const enrollStudent = async (studentId: number) => {
    ...
    await fetch("/api/enrollments", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ studentId, groupId: selectedGroup.id }),
    });
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${SUBJECT_COLORS[group.subject] || SUBJECT_COLORS["أخرى"]}`}>
                    {group.subject}
                  </span>
                  {group.grade && (
                    <span className="mr-1 text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600">
                      {group.grade}
                    </span>
                  )}
                </div>
                <div className="text-left">
                  <p className="font-bold text-indigo-600 text-sm">
                    {parseFloat(group.monthlyFee).toLocaleString("ar-EG")} ج.م
                  </p>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                <div className="flex items-center gap-1 text-slate-500 text-sm">
                  ...
                  {group.maxStudents && (
                    <span className="text-slate-300">/ {group.maxStudents}</span>
                  )}
                </div>
                <div className="flex gap-1">
                  <button
                    onClick={(e) => { e.stopPropagation(); openEdit(group); }}
                    className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center text-xs"
                  >
                    ✏️
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); handleDelete(group.id); }}
                    className="w-8 h-8 bg-red-50 rounded-lg flex items-center justify-center text-xs"
                  >
                    🗑️
                  </button>
                </div>
          ...
          >
            <div className="p-6 pb-8">
              ...
              <h3 className="text-lg font-bold text-slate-800 mb-4">
                {editMode ? "✏️ تعديل المجموعة" : "👥 مجموعة جديدة"}
              </h3>
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="مثال: مجموعة الرياضيات A"
                    ...
                    {form.schedules.map((s, i) => (
                      <div key={i} className="flex items-center gap-2 bg-slate-50 rounded-xl p-2">
                        ...
                        {form.schedules.length > 1 && (
                          <button onClick={() => removeSchedule(i)} className="text-red-400 text-sm">✕</button>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <span className={`text-sm px-2 py-0.5 rounded-full ${SUBJECT_COLORS[selectedGroup.subject] || ""}`}>
                    {selectedGroup.subject}
                  </span>
                </div>
                <span className="font-bold text-indigo-600">
                  {parseFloat(selectedGroup.monthlyFee).toLocaleString("ar-EG")} ج.م
                </span>
              </div>
                  ...
                  {selectedGroup.students.map((s) => (
                    <div key={s.enrollmentId} className="flex items-center gap-3 bg-slate-50 rounded-xl p-3">
                      <div
                        ...
                        style={{ background: `hsl(${(s.studentId * 47) % 360}, 65%, 55%)` }}
                      >
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => { setShowDetails(false); openEdit(selectedGroup); }}
