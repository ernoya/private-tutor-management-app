...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    ...
    fetchGroups();
    fetchStudents();
  }, [fetchPayments, fetchGroups, fetchStudents]);
  ...
  const updateStatus = async (p: Payment, newStatus: string) => {
    await fetch("/api/payments", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id: p.id, status: newStatus, amount: parseFloat(p.amount), notes: p.notes }),
    });
    fetchPayments();
  };
  ...
  // Bulk generate payments for all students in a group
  const handleBulkGenerate = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/groups/${bulkForm.groupId}`);
      ...
      for (const student of groupData.students || []) {
        ...
        const existing = payments.find(
          (p) => p.studentId === student.studentId &&
            p.groupId === parseInt(bulkForm.groupId) &&
            p.month === bulkForm.month
        );
        if (!existing) {
          await fetch("/api/payments", {
            method: "POST",
            ...
            body: JSON.stringify({
              ...
              groupId: parseInt(bulkForm.groupId),
              amount: parseFloat(group.monthlyFee),
  ...
  const filtered = payments.filter(
    (p) => filterStatus === "الكل" || p.status === filterStatus
  );
  ...
  const selectedGroupFee = groups.find((g) => g.id === parseInt(form.groupId))?.monthlyFee;
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-2 rounded-xl bg-amber-100 text-amber-700 text-sm font-medium"
          >
            📋 إنشاء جماعي
          </button>
          <button
            onClick={() => {
              setForm({
                studentId: "", groupId: "", amount: "",
                month: selectedMonth, status: "مدفوع", notes: "",
              });
              setShowModal(true);
            }}
            className="flex items-center gap-1 px-3 py-2 rounded-xl text-white text-sm font-medium shadow-md"
      ...
      <div className="card p-3 flex items-center justify-between">
        <button onClick={() => changeMonth(-1)} className="w-9 h-9 bg-slate-100 rounded-lg flex items-center justify-center">
          ▶
        </button>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="card p-4 bg-red-50">
          ...
          <p className="text-xl font-bold text-red-700">
            {totalUnpaid.toLocaleString("ar-EG")} ج.م
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-slate-400 text-sm mt-1">
            استخدم &quot;إنشاء جماعي&quot; لإنشاء مدفوعات لمجموعة كاملة
          </p>
        </div>
      ) : (
            ...
            <div key={p.id} className="card p-4">
              <div className="flex items-center gap-3">
                <div
                  ...
                  style={{ background: `hsl(${(p.studentId * 47) % 360}, 65%, 55%)` }}
                >
              ...
              <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                {p.status !== "مدفوع" && (
                  <button
                    onClick={() => updateStatus(p, "مدفوع")}
                    className="flex-1 py-1.5 rounded-lg bg-green-50 text-green-700 text-xs font-medium"
                  >
                    ✅ تم الدفع
                  </button>
                )}
                {p.status !== "جزئي" && (
                  <button
                    onClick={() => updateStatus(p, "جزئي")}
                    className="flex-1 py-1.5 rounded-lg bg-yellow-50 text-yellow-700 text-xs font-medium"
                  >
                    🔶 جزئي
                  </button>
                )}
                {p.status !== "غير مدفوع" && (
                  <button
                    onClick={() => updateStatus(p, "غير مدفوع")}
                    className="flex-1 py-1.5 rounded-lg bg-red-50 text-red-700 text-xs font-medium"
                  >
                    ❌ غير مدفوع
                  </button>
                )}
                <button
                  ...
                  className="w-8 py-1.5 rounded-lg bg-slate-100 text-slate-500 text-xs"
                >
                  🗑️
                </button>
              </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={() => setShowModal(false)}>
          <div
            className="bg-white rounded-t-3xl w-full max-w-[430px] p-6 pb-8 slide-up overflow-y-auto"
          ...
          >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setForm({ ...form, groupId: e.target.value, amount: g ? parseFloat(g.monthlyFee).toString() : form.amount });
                  }}
                  className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-400"
                >
                  ...
                  {["مدفوع", "غير مدفوع", "جزئي"].map((s) => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-xl text-xs font-medium border-2 transition-all ${
                        form.status === s
                          ? s === "مدفوع" ? "border-green-500 bg-green-50 text-green-700"
                          : s === "جزئي" ? "border-yellow-500 bg-yellow-50 text-yellow-700"
            ...
            <div className="flex gap-2 mt-4">
              ...
              <button
                onClick={handleSave}
                disabled={saving || !form.studentId || !form.groupId || !form.amount}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={() => setShowBulkModal(false)}>
          ...
          >
            ...
            <p className="text-sm text-slate-500 mb-4">
              سيتم إنشاء دفعة &quot;غير مدفوع&quot; لجميع طلاب المجموعة المختارة
            </p>
            <div className="space-y-3">
              <div>
                <label className="text-xs font-medium text-slate-500 mb-1 block">المجموعة *</label>
                ...
                >
                  ...
                  {groups.map((g) => (
                    <option key={g.id} value={g.id}>
                      {g.name} - {parseFloat(g.monthlyFee).toLocaleString("ar-EG")} ج.م/شهر
                    </option>
                  ))}
                </select>
              </div>
              ...
              {bulkForm.groupId && (
                <div className="bg-amber-50 rounded-xl p-3 text-sm text-amber-700">
                  💡 سيتم إنشاء دفعة بمبلغ{" "}
                  <strong>
                    {parseFloat(groups.find((g) => g.id === parseInt(bulkForm.groupId))?.monthlyFee || "0").toLocaleString("ar-EG")} ج.م
                  </strong>{" "}
                  لكل طالب في هذه المجموعة
                </div>
              )}
            </div>
            <div className="flex gap-2 mt-4">
              ...
              <button
                ...
                disabled={saving || !bulkForm.groupId}
                className="flex-1 py-3 rounded-xl text-white font-medium text-sm disabled:opacity-50"
                style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)" }}
              >
