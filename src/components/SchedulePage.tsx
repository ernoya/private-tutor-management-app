...
export default function SchedulePage() {
  ...
  const monthLabel = new Date(viewDate + "T00:00:00").toLocaleDateString("ar-EG", {
    year: "numeric",
    month: "long",
  });
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1" onClick={() => openAttendance(session)}>
                        ...
                        <span
                          ...
                          style={{
                            background: SUBJECT_COLORS[session.subject] + "20",
                            color: SUBJECT_COLORS[session.subject],
                          }}
                        >
                          {session.subject}
                        </span>
                      ...
                      <div className="flex flex-col gap-1 items-end">
                        <button
                          ...
                          className="text-xs bg-indigo-50 text-indigo-600 px-2 py-1 rounded-lg font-medium"
                        >
                          الحضور
                        </button>
                        <button
                          onClick={() => deleteSession(session.id)}
                          className="text-xs bg-red-50 text-red-400 px-2 py-1 rounded-lg"
                        >
                          حذف
                        </button>
                      </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="أي ملاحظة عن الحصة..."
                  className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-400"
                />
              </div>
            </div>
          ...
          >
            <div className="p-6">
              ...
              <p className="text-sm text-slate-500 mb-4">
                {formatDate(selectedSession.sessionDate)} | {selectedSession.startTime} – {selectedSession.endTime}
              </p>
                  ...
                  {attendanceRecords.map((record) => (
                    ...
                    >
                      <div
                        ...
                        style={{ background: `hsl(${(record.studentId * 47) % 360}, 65%, 55%)` }}
                      >
                        {record.studentName.charAt(0)}
                      </div>
                      <span className="flex-1 font-medium text-slate-800 text-sm text-right">
                        {record.studentName}
                      </span>
                      <span className="text-xl">{STATUS_ICONS[record.status]}</span>
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                        record.status === "حاضر" ? "bg-green-200 text-green-800" :
                        record.status === "غائب" ? "bg-red-200 text-red-800" :
